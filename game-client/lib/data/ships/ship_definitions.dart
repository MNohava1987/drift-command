import 'package:drift_command/core/models/ship_data.dart';

/// Canonical ship definitions keyed by data ID.
/// These are used everywhere: scenario loader, systems, rendering.
const kShipDefinitions = <String, ShipData>{
  'flagship': ShipData(
    id: 'flagship',
    displayName: 'Flagship',
    role: ShipRole.flagship,
    massClass: MassClass.capital,
    maxAcceleration: 12.0,
    turnRate: 0.4,
    commandLatencyMod: 0.8,
    sensorRange: 400.0,
    weaponRange: 120.0,
    maxDurability: 200.0,
    roleTags: [RoleTag.commandSource, RoleTag.directFire],
  ),
  'command_relay': ShipData(
    id: 'command_relay',
    displayName: 'Command Relay',
    role: ShipRole.commandRelay,
    massClass: MassClass.medium,
    maxAcceleration: 18.0,
    turnRate: 0.8,
    commandLatencyMod: 0.6,
    sensorRange: 350.0,
    weaponRange: 80.0,
    maxDurability: 80.0,
    roleTags: [RoleTag.commandRelay],
  ),
  'heavy_line': ShipData(
    id: 'heavy_line',
    displayName: 'Heavy Line Ship',
    role: ShipRole.heavyLine,
    massClass: MassClass.heavy,
    maxAcceleration: 8.0,
    turnRate: 0.15,
    commandLatencyMod: 1.5,
    sensorRange: 200.0,
    weaponRange: 150.0,
    maxDurability: 120.0,
    roleTags: [RoleTag.directFire, RoleTag.pointDefense],
  ),
  'light_escort': ShipData(
    id: 'light_escort',
    displayName: 'Light Escort',
    role: ShipRole.lightEscort,
    massClass: MassClass.light,
    maxAcceleration: 30.0,
    turnRate: 1.5,
    commandLatencyMod: 0.9,
    sensorRange: 280.0,
    weaponRange: 100.0,
    maxDurability: 50.0,
    roleTags: [RoleTag.directFire, RoleTag.screening, RoleTag.pointDefense],
  ),
  'strike_carrier': ShipData(
    id: 'strike_carrier',
    displayName: 'Strike Carrier',
    role: ShipRole.strikeCarrier,
    massClass: MassClass.medium,
    maxAcceleration: 15.0,
    turnRate: 0.6,
    commandLatencyMod: 1.2,
    sensorRange: 350.0,
    weaponRange: 200.0,
    maxDurability: 90.0,
    roleTags: [RoleTag.missile],
  ),
  'fast_raider': ShipData(
    id: 'fast_raider',
    displayName: 'Fast Raider',
    role: ShipRole.fastRaider,
    massClass: MassClass.light,
    maxAcceleration: 35.0,
    turnRate: 2.0,
    commandLatencyMod: 0.7,
    sensorRange: 300.0,
    weaponRange: 90.0,
    maxDurability: 40.0,
    roleTags: [RoleTag.directFire, RoleTag.flanking],
  ),
};
